...
//    SEND_BUF(rece_buf);
    while(1)
    {
        ...
        {
            if(NRF24L01_RxPacket()==0)  //设置为接收模式  接收完毕
            {
                if(	RevTempDate0[1]==0x01)		 //第1位以后是收到的命令数据，rece_buf[0]是数据位数长度
                    LED1=!LED1;
                if(	RevTempDate1[1]==0x02)	 	 //第1位以后是收到的命令数据，rece_buf[0]是数据位数长度
                    LED0=!LED0;
                if(	RevTempDate2[1]=='3')	 	 //第1位以后是收到的命令数据，rece_buf[0]是数据位数长度
                    LED0=1;
                if(	RevTempDate3[1]=='4')	 	 //第1位以后是收到的命令数据，rece_buf[0]是数据位数长度
                    LED1=1;
            }
        }
