...
// IO CONFIG 
	DPS = 0x00 ;
	P0DIR = 0xD2 ;   // P05 output, P01 & P02 as uart port 
	P0ALT = 0x06 ; 
	P0EXP = 0x00 ; 
//	CLKCTL = 0x30;
//	P1DIR = 0x03 ;
//	P1ALT = 0x0F ; // enable RF IOs
	...
	while(1)
	{  	
		RFCON = RFCON & 0xFE ;  // CE = 0 ;		   
		spi_rw_reg(FLUSH_TX,0);
     	spi_write_buf(W_ACK_PAYLOAD + PIPE0,RX_BUF, TX_PLOAD_WIDTH);  // 将收到的数据做为ACKPayload发送出去
	    RFCON = RFCON | 0x01 ;  //CE = 1 ;
...
void rx_mode(void)
{
	...
	spi_rw_reg(W_REGISTER + FEATURE, 0x06);                             // 使能PAYLOAD with ACK AND DYNPD
   	spi_rw_reg(W_REGISTER + DYNPD, 0x01);								   //开启DPL_P0	 

  	spi_rw_reg(W_REGISTER + RF_CH, 0x7B);                 				// 选择射频通道0x40
  	spi_rw_reg(W_REGISTER + RX_PW_P0, TX_PLOAD_WIDTH);  				// 接收通道0选择和发送通道相同有效数据宽度

  	spi_rw_reg(W_REGISTER + RF_SETUP, 0x0E);            				// 数据传输率2Mbps,0dbm TX power
