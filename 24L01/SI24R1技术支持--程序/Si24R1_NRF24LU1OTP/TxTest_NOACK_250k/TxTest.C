...
void rx_mode(void)
{
	...
	spi_write_buf(W_REGISTER + RX_ADDR_P0, TX_ADDRESS, TX_ADR_WIDTH);	// 接收设备接收通道0使用和发送设备相同的发送地址
  	...
  	spi_rw_reg(W_REGISTER + RF_SETUP, 0x27);            				// 数据传输率250kbps,7dbm TX power
...
void tx_mode(uchar *buf)
{
	...
	spi_rw_reg(W_REGISTER + SETUP_AW, 0x03);  						 		// 5 byte Address width
  	...
  	spi_rw_reg(W_REGISTER + SETUP_RETR, 0x1F);  						 	// 自动重发。     自动重发延时等待500us+86us，自动重发15次
  	spi_rw_reg(W_REGISTER + RF_CH, 0x40);         					     	// 选择射频通道0x40

  	spi_rw_reg(W_REGISTER + RF_SETUP, 0x27);   						 	    // 数据传输率250kbps(**与24L01不同**),7dbm TX power
