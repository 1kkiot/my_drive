...
//----------------------------   全局变量   ----------------------------------
uchar code TX_ADDRESS[TX_ADR_WIDTH] = {0x0A,0x0A,0x0A,0x0A,0x0A};  // 定义一个静态发送地址
...
*/
 ////////////////////////
   		//////////////跳频表
	uchar ch_pointer;
	uchar CH_TAB[4]={0, 30, 80, 123 }	;  //frequency channel table
	...
	while(1)
	{
		...
		{   
		  ...
		  //////////////////////////////////////// 
			...
			printf("TXData:\n");
			printf("==#%d==#RFCH=%d\n",(uint)TX_BUF[0],(uint)CH_TAB[ch_pointer]);	
			for(i = 1; i < TX_PLOAD_WIDTH; i++)
			{
			...
			printf("\n==\n\n");
		       ...
		       ch_pointer=0;
			   CE=0	;
		       spi_rw_reg(W_REGISTER + RF_CH, (uint)CH_TAB[ch_pointer]); 
			   CE=1;
			   TX_BUF[0] = counter++;	
			   ////////////////////////////
			   //light_counter+=1;
	...
	//	 printf("CH=%d\n", (uint)CH_TAB[ch_pointer]); 
		 spi_rw_reg(W_REGISTER + STATUS,status_reg);
		    ///////////////////////////////////
		      ...
		      ch_pointer=0;
			  CE=0	;
		      spi_rw_reg(W_REGISTER + RF_CH, CH_TAB[ch_pointer]);  
			  spi_rw_reg(FLUSH_TX);	/////NOTES!!!!
			  CE=1;
			  //////////////////////////////////////
		}
...
void tx_mode(uchar *buf)
{
	...
	spi_rw_reg(W_REGISTER + SETUP_AW, 0x03);  						 		// 5 byte Address width
  	...
  	spi_rw_reg(W_REGISTER + EN_RXADDR, 0x01);   						 	// 使能接收通道0
  	spi_rw_reg(W_REGISTER + SETUP_RETR, 0x0c);  						 	// 使能自动重发 250us 15次重发
  	spi_rw_reg(W_REGISTER + RF_CH, CH_TAB[ch_pointer]);         					     	// 选择射频通道0x40
  	spi_rw_reg(W_REGISTER + RF_SETUP, 0x07);   						 	    // 数据传输率1Mbps(**与24L01不同**),7dbm TX power
	spi_rw_reg(W_REGISTER + SETUP_RETR, 0x09);   						 	    // 数据传输率1Mbps(**与24L01不同**),7dbm TX power
